...
const toTitleCase = (arr) => {
  // Solution code here...
  returnedArray = arr.map(value => value.charAt(0).toUpperCase() + value.slice(1));
...
let biggerThanLuke = (arr) => {
  // Solution code here...
    const massOfLuke = 77;
    return arr
      .filter(value => parseInt(val.mass) > massOfLuke)
      .map(val => value.name)
      .join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  let returnedValue = arr.sort((a, b) => (a[property] < b[property] ? -1 : 1));
